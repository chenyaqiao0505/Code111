...
def write_excel( sheet_name, value,filepath):#写表头
  index = len(value)
...
#       print(j,'行',j+worksheet.nrows,'列',1,value[0][j])
#       new_worksheet.write(j + worksheet.nrows, 1, value[0][j])  # 追加写入数据，注意是从i+rows_old行开始写入
...
def CodeBar(path, value):
    ...
    for j in range(0, len(value1[0])):
        print(j, '行', j + worksheet.nrows, '列', 0, value1[0][j])
        new_worksheet.write(j + worksheet.nrows, 0, value1[0][j])  # 追加写入数据，注意是从i+rows_old行开始写入
    new_workbook.save(path)  # 保存工作簿
    print("xls格式表格【追加】写入数据成功！")
...
list = ['pass', 'pass', [1, 3, 5, 7], [1, 3, 5, 7],
...
def StartButton():
    ...
    # text.insert(result)
    # print("开始")
    # my_file = Path("C:\\Users\\admin\\Desktop\\Result.xls")
    if os.path.exists(book_name):
        print('该文件已存在，可以直接进行测试~！ ')
    else:
        write_excel(sheet_name, value_title, book_name)
        write_excel_append(book_name, handle(list))
...
def touch(event):
    qq = UDP.CheckNum()
    list = []
    name = [ConfP1, ConfP2, ConfP3, ConfP4, ConfP5, ConfP6, ConfP7, ConfP8,
    ConfP9, ConfP10, ConfP11, ConfP12, ConfP13, ConfP14, ConfP15, ConfP16]
    if qq != len(list):
...
def ui_process():
    ...
    global ConfP1, ConfP2, ConfP3, ConfP4, ConfP5, ConfP6, ConfP7, ConfP8, ConfP9, ConfP10, ConfP11, ConfP12, ConfP13, ConfP14, ConfP15, ConfP16
    global ConfIn2, e12
