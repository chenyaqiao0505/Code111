from Udp_test import  udpclient
import time
import threading 
...
class udpdevicedi:
    ...
    def Read(self,value):
        ...
        returned_result = w1.control(msg, True)

        resu = [] # 接收结果不正确的值
        r1 = []   # 接受结果的列表
        msg_index = 0     # 使用msg_index来限制位数
        for x in bytes(returned_result):
            msg_index +=1
            if msg_index !=1 and msg_index !=2:     #去掉前两位
                resu.append(x ^ value)      #二进制求异或操作
                a = x ^ value
                if a ==0:
