...
Label(root, fg='green', text='DI输入', font=('KaiTi', 15, 'bold')).place(x=170, y=110)
...
def StartButton():
    ...
    time.sleep(1)
    book_name = 'DI测试'+datetime.datetime.now().strftime("%Y%m%d%H%M%S") +'.xls'
    if P21.get() == '':
        ...
        if os.path.exists(book_name):
                ...
                write_excel_append(book_name, finallist)
        else:
...
def write_excel_append(path, value):
    ...
    for j in range(0, len(value)):
        new_worksheet.write(j+1,1, DealStr(value[j]))
...
def finalhandle(list):      #前后换位
    handlelist1 = []
    handlelist2 = []
    for o in range(len(list)):          #o表示的是遍历list元素
        if o % 2 != 1 and type(list[o]) != type('pass'):        #当位置为偶数位的时候
            for i in range(len(list[o])):
                a = int(DealStr(list[o][i]))
                a = 16 - a
                list[o][i] = a
            
    for o in range(len(list)):
        if o % 2 != 0 and type(list[o]) != type('pass'):        #当位置为偶数位的时候
            for i in range(len(list[o])):
                a = int(DealStr(list[o][i]))
                a = 8 - a
                list[o][i] = a
