...
class Server(models.Model):
    ip                  = models.CharField('管理ip',max_length=15,db_index=True,unique=True,help_text='管理ip') #设置索引值db_index
    ...
    manufacturer        = models.ForeignKey(Manufacturer,on_delete=None,verbose_name='制造商',help_text='制造商')
    model_name          = models.ForeignKey(ProductModel,on_delete=None,verbose_name='服务型号',help_text='服务型号')
    rmt_card_ip         = models.CharField('远程管理卡ip',max_length=15,db_index=True,unique=True,help_text='远程管理卡ip')
    idc                 = models.ForeignKey(Idc,null=True,on_delete=None,verbose_name='所在机房',help_text='所在机房')
    cabinet             = models.ForeignKey(Cabinet,null=True,on_delete=None,verbose_name='所在机柜',help_text='所在机柜')
...
class NetworkDevice(models.Model):
    ...
    host                = models.ForeignKey(Server,on_delete=None,verbose_name='所在服务器',help_text='所在服务器')
    remark              = models.CharField('备注',max_length=200,help_text='备注')
...
class IP(models.Model):
    ...
    device              = models.ForeignKey(NetworkDevice,on_delete=None,verbose_name='所在网卡',help_text='所在网卡')
