...
import os
import datetime
from UDPdeviceDO import udpdevicedo
import tkinter.messagebox
import threading
...
Label(root, fg='green', text='DO输出', font=('KaiTi', 15, 'bold')).place(x=170, y=110)
...
def StartButton():
    ...
    if P21.get() == '':
        ...
        if os.path.exists(book_name):
            ...
            write_excel_append(book_name, handle(result))
        else:
...
def finalhandle(list):      #解码
    for o in range(len(list)):
        if type(list[o]) != type('pass'):
            for q in range(len(list[0])):
                list[o][q] += 1
    return list
