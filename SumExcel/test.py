...
file_dir = 'E:\\Foxmail\\20181119发货输出板卡测试记录\\20181114'
...
def all_to_one(root, allFile_url, file_dir,files, file_name, title=None):
...
def clear_data(file_dir,file_name):
    # file_dir + '\\' +
...
sum_files = len(get_allfile_url(root,files))
e1.insert(END, '文件数量:', )
e1.insert(END,sum_files,'\n', '\n')
valuebtu = Button(win, text="写入地址", command=getuser)
...
btu = Button(win, text='开始汇总',command=lambda: all_to_one(root, allFile_url,file_dir=file_dir, files=files, file_name=file_name, title=title))
